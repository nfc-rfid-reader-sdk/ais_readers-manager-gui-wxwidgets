...
void SendRealTimeEvent(){
  ...
  to_file +=  to_string(log_index) + "           |" + dbg_action2str(static_cast<E_CARD_ACTION>(log_action)) + "|    " + to_string(log_reader_id) + "       | "
   ...
   "       |" + dbg_GMT2str(timestamp) + "  | " + "R" + '\n';
    ...
    to_sendPHP = "logindex=" + to_string(log_index) + "&logaction=" + dbg_action2str(static_cast<E_CARD_ACTION>(log_action)) + "&readerid=" + to_string(log_reader_id)
     + "&cardid=" + to_string(log_card_id) + "&systemid=" + to_string(log_system_id) + "&nfcuid=" + uid + "&uidlength=" + to_string(nfc_uid_len) + "&timestamp=" + dbg_GMT2str(timestamp) + "&Ror=L";
...
if (gui_on == 0){
    ...
    AppLog << endl << ReadWhitelistFromReader(DEVICES_HANDLE[ID]);
    }else
     {
            for (int i = 0; i < device_count; i++)
            {
                cout << endl << "Device number [" << DeviceID[i] << "]" << " whitelist : ";
                AppLog << endl << "Device number [" << DeviceID[i] << "]" << " whitelist : ";
                cout << endl << ReadWhitelistFromReader(handle[i]);
                AppLog << endl << ReadWhitelistFromReader(handle[i]);
            }
...
if (gui_on == 0){
     ...
     AppLog << endl << ReadBlacklistFromReader(DEVICES_HANDLE[ID]);
            ...
            for (int i = 0; i < device_count; i++)
            {
                cout << endl << "Device number [" << DeviceID[i] << "]" << " blacklist : ";
                AppLog << endl << "Device number [" << DeviceID[i] << "]" << " blacklist : ";
                cout << endl << ReadBlacklistFromReader(handle[i]);
                AppLog << endl << ReadBlacklistFromReader(handle[i]);
            }
...
void GetLogsByTime(){
    ...
    /**** TESTING ***/ //TODO Mora funkcija za unos vremena da se promeni Krxy!
...
*/
    ...
    cin >> time1;
    cin >> date1;
    AppLog << time1 << " " << date1 << endl;
    cout << "Enter time to when to stop reading (hours:minutes:seconds day.month.year) : " << endl;
    AppLog << "Enter time to when to stop reading (hours:minutes:seconds day.month.year) : " << endl;
 cin >> time2;
 cin >> date2;
    AppLog << time2 << " " << date2 << endl;

    from_str = time1 + " " + date1;
